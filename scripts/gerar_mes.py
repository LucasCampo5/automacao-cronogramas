...
def escolher_aba_base(wb):
    """
    Escolhe a última aba real, ignorando abas de teste/temporárias.
    """
...
def pintar_coluna_planejamento(ws, config, coluna, cor_cabecalho, dia_semana=None, dia_existe=True):
    ...
    linhas_ignorar = config.get("linhas_ignorar_preenchimento", [])
    ...
    if not regras:
        for linha in range(linha_inicio, linha_fim + 1):
            if linha in linhas_ignorar:
                continue

            ws.range((linha, coluna)).color = cor_cabecalho
    ...
    if not dia_existe or dia_semana in [5, 6]:
        for linha in range(linha_inicio, linha_fim + 1):
            ...
            ws.range((linha, coluna)).color = COR_CINZA

        return

    # Dia útil com regras específicas:
    # pinta somente as atividades configuradas
    for linha in range(linha_inicio, linha_fim + 1):
        if linha in linhas_ignorar:
            continue
...
def limpar_dia_inexistente(ws, config, coluna):
    """
    Limpa conteúdo dos dias que não existem no mês.
    Exemplo:
    - dia 31 em novembro;
    - dias 29, 30 e 31 em fevereiro comum.
    """
    ...
    # Limpa número do dia e dia da semana
    ws.range((linha_numero_dia, coluna)).value = ""
    ws.range((linha_dia_semana, coluna)).value = ""

    # Limpa área de atividades
    for linha in range(linha_inicio, linha_fim + 1):
        if linha in linhas_ignorar:
            continue

        ws.range((linha, coluna)).value = ""


def limpar_colunas_extras(ws, config, ultimo_dia):
    """
    Limpa colunas extras que podem ter sobrado da aba modelo.
    Só funciona se a planilha tiver 'coluna_limite_limpeza' no config.
    """

    coluna_inicial = config["coluna_inicial_dias"]
    coluna_final_mes = coluna_inicial + ultimo_dia - 1

    coluna_limite = config.get("coluna_limite_limpeza")

    if not coluna_limite:
        return

    if coluna_limite <= coluna_final_mes:
        return
    ...
    for coluna in range(coluna_final_mes + 1, coluna_limite + 1):
        ws.range((linha_numero_dia, coluna)).value = ""
        ws.range((linha_dia_semana, coluna)).value = ""
        ...
        ws.range((linha_execucao, coluna)).value = ""
        ws.range((linha_verificacao, coluna)).value = ""
...
def atualizar_calendario(ws, config, mes, ano):
    ...
    # Limpa possíveis sobras de colunas extras, se estiver configurado
    limpar_colunas_extras(ws, config, ultimo_dia)
...
def processar_planilha(prefixo, meses_para_gerar, ano, app):
    config = CONFIG_PLANILHAS[prefixo]

    caminho_entrada = encontrar_arquivo(prefixo)

    if len(meses_para_gerar) == 1:
        nome_aba_unica = nome_aba_mes(meses_para_gerar[0], ano)
        nome_saida = f"{prefixo} - {nome_aba_unica}{caminho_entrada.suffix}"
    else:
        nome_saida = f"{prefixo} - ANO {ano}{caminho_entrada.suffix}"
    ...
    wb = None
    nome_aba_temp = "MODELO_TEMP_PYTHON"
    ...
    try:
        ...
        for mes in meses_para_gerar:
            ...
            aba_modelo_temp.api.Copy(After=wb.sheets[-1].api)
        ...
        if nome_aba_temp in [sheet.name for sheet in wb.sheets]:
            wb.sheets[nome_aba_temp].delete()
...
def main():
    ...
    try:
        ano = int(input("Digite o ano. Exemplo 2027: "))
    except ValueError:
        print("Ano precisa ser número.")
        return
    ...
    if modo == "1":
        ...
        meses_para_gerar = [mes]

    else:
        meses_para_gerar = list(range(1, 13))
    ...
    try:
        for prefixo in prefixos:
            processar_planilha(prefixo, meses_para_gerar, ano, app)
